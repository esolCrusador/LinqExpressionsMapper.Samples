...
namespace LinqExpressionsMapper.Samples.AllExamples
{
    public static class CultureResolveExample
    {
        public static void ShowCourses(SchoolContext context)
        {
            ...
            var courses = context.Courses.Project().To<CourseModel>().SelectWith<Culture>(Culture.DE).ToList();

            //Students with localized course names.
            var studentCourses = context.Students.Project().To<StudentWithCourses>().SelectWith<Culture>(Culture.ES).ToList();
        }
